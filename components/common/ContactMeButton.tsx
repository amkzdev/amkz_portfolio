...
const ContactInfo = ({ icon, title, link }: { icon: StaticImageData, title: string, link: string }) => {
    return <a href={link} target='_blank' className='flex flex-row items-center gap-4 p-4 justify-start hover:bg-gray-50 cursor-pointer'>
    ...
    if (typeof document != 'undefined')
        ...
        return (
            ...
            <>
                ...
                <button className='bg-light-primary dark:bg-dark-primary p-2 flex  flex-row dark:text-black text-white font-semibold rounded px-8 items-center gap-2' onClick={() => setOpen(!open)}>
                ...
                {createPortal(
                    <ClickAwayListener onClickAway={() => setOpen(false)}>
                        <div className={`fixed whitespace-nowrap flex left-1/2 -translate-x-1/2 flex-col z-20  p-2 bg-white rounded-lg transition-all duration-300   ${open ? ' top-1/2 -translate-y-1/2' : ' top-full '}`}>
